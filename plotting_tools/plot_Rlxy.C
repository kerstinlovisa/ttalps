...
//                                logy   logx   rebinned  rebin   xMin    xMax    yMin    yMax    xlabel                       ylabel
  ...
  auto legend_linestyle = new TLegend(0.40, 0.52, 0.65, 0.64);
  set_legend_layout(legend_linestyle);
  legend_linestyle->SetMargin(0.2);
  ...
  legend_linestyle->AddEntry((TObject*)0, "resonant", "l");
  legend_linestyle->AddEntry(hist_tmp, "non-resonant", "l");
      ...
      for (auto prefix : prefixes){
        for (auto category : categories) {
          ...
          if(find(in_legend.begin(), in_legend.end(), file_name) == in_legend.end()){
            if(signal){
              if(line==kSolid) legend_sig->AddEntry(hist, title.c_str(), "l");
            }
